...
function openModal() {
  ...
  modalCaption.textContent =
    galleryItems[currentIndex].querySelector("img").alt;
...
function showCustomAlert(message, isSuccess = true) {
  ...
  alertBox.style.background = isSuccess ? "#2b1e13" : "#8b1e1e";
...
(function () {
  emailjs.init("G4zp2UwqNnxM_kJfp"); 
})();

// Utility: check email format
...
if (contactForm) {
  contactForm.addEventListener("submit", function (e) {
    ...
    emailjs.sendForm("service_7ioe4w1", "template_ae6v4no", this).then(
      function () {
        showCustomAlert("✅ Message sent successfully!");
        contactForm.reset();
      },
      function (error) {
        showCustomAlert("❌ Failed to send message. Error: " + JSON.stringify(error));
      }
    );
  });
}
